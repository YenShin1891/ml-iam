...
# Populated at runtime by load_and_process_data() from the actual dataset.
# Sorted for deterministic embedding indices across runs.
REGION_CATEGORIES: list = []
REGION_CODE_TO_LABEL: dict = {}
...
# When True, keep rows where at least one target is observed (partial coverage).
# LSTM/TFT use masked loss; XGBoost trains per-target models dropping NaN
