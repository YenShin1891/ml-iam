...
from .tft_trainer import (
    predict_tft, _single_gpu_env, _teardown_dist, _load_tft_checkpoint,
    _from_train_template, _create_infer_trainer, _default_num_workers,
    _flatten_predictions, _normalize_index_df
)
...
def _predict_early_window(session_state: dict, run_id: str) -> Tuple[np.ndarray, pd.DataFrame]:
    ...
    with _single_gpu_env():
        ...
        try:
            test_dataset = _from_train_template(train_template, early_test_data, mode="eval")
        except Exception as e:
...
def _predict_late_window(session_state: dict, run_id: str) -> tuple[np.ndarray, pd.DataFrame]:
    ...
    with _single_gpu_env():
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load dataset template for late window: {e}")
        ...
        try:
            ...
            test_dataset = _from_train_template(train_template, late_test_data, mode="predict")
            ...
            try:
                ...
                test_dataset = _from_train_template(train_template, late_test_data, mode="eval")
            except Exception as e2:
                raise RuntimeError(f"Failed to build late window test dataset with both predict ({e1}) and eval ({e2}) modes")
        except Exception as e:
            raise RuntimeError(f"Failed to build late window test dataset: {e}")
